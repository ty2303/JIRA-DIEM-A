...
function removeGoogleAuthTestUsers() {
  db.users = db.users.filter(
    (user) => !String(user.email).endsWith("@google-callback.test"),
  );
}
...
function waitForFrame(ws, timeoutMs = 5000) {
  return new Promise((resolve, reject) => {
    ...
    const onClose = () => {
      cleanup();
      reject(new Error("WebSocket closed before frame"));
    };
...
describe("Auth - local", () => {
  ...
  test("GET /api/auth/google/callback rejects invalid state before calling Google", async () => {
    ...
    await withServer(async (port) => {
      ...
      assert.equal(
        body.message,
        "State Google OAuth không hợp lệ hoặc đã hết hạn",
      );
  ...
  test("GET /api/auth/google/callback maps invalid_grant and rejects replayed state", async () => {
    ...
    await withServer(async (port) => {
      ...
      assert.equal(
        firstBody.message,
        "Google authorization code không hợp lệ hoặc đã hết hạn",
      );
...
describe("Products", () => {
  ...
  test("GET /api/products/:id returns a single product", async () => {
    await withServer(async (port) => {
      const res = await fetch(
        `http://127.0.0.1:${port}/api/products/prod-iphone-15`,
      );
      const body = await res.json();
  ...
  test("GET /api/products/:id returns 404 for unknown id", async () => {
    await withServer(async (port) => {
      const res = await fetch(
        `http://127.0.0.1:${port}/api/products/does-not-exist`,
      );
  ...
  test("GET /api/products filters by category", async () => {
    await withServer(async (port) => {
      const res = await fetch(
        `http://127.0.0.1:${port}/api/products?categoryId=cat-iphone`,
      );
      const body = await res.json();
  ...
  test("GET /api/products searches by name", async () => {
    await withServer(async (port) => {
      const res = await fetch(
        `http://127.0.0.1:${port}/api/products?search=iPhone`,
      );
      const body = await res.json();
...
describe("Cart", () => {
  ...
  test("POST /api/cart/items adds a product to cart", async () => {
    await withServer(async (port) => {
      ...
      assert.equal(res.status, 200);
      assert.ok(
        body.data.items.some((item) => item.productId === "prod-iphone-15"),
      );
    });
  });
...
describe("Reviews", () => {
  ...
  test("DELETE /api/reviews/:id removes the review", async () => {
    ...
    await withServer(async (port) => {
      const res = await fetch(
        `http://127.0.0.1:${port}/api/reviews/review-del-test`,
        {
          method: "DELETE",
          headers: { Authorization: "Bearer demo-token" },
        },
...
describe("Reviews - analysis model", () => {
  ...
  test("seed reviews have analysisResult null by default", async () => {
    await withServer(async (port) => {
      const res = await fetch(
        `http://127.0.0.1:${port}/api/reviews?productId=prod-iphone-15`,
      );
      ...
      assert.ok(review, "review-1 should exist");
      assert.strictEqual(review.analysisResult, null);
    });
  });

  test("POST /api/reviews new review has analysisResult null", async () => {
    await withServer(async (port) => {
      const res = await fetch(`http://127.0.0.1:${port}/api/reviews`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          productId: "prod-galaxy-s25",
          rating: 5,
          comment: "Cam ung muot, man hinh dep, pin trau",
          images: [],
        }),
      });
      const body = await res.json();
  ...
  test("review analysisResult stores positive sentiment data", async () => {
    const analysisResult = {
      sentiment: "positive",
      sentimentScore: 0.95,
      qualityScore: 0.88,
      flags: [],
      summary: "Nguoi dung hai long ve san pham, phan hoi tich cuc.",
    ...
    await withServer(async (port) => {
      ...
      assert.ok(found);
      assert.equal(found.analysisResult.sentiment, "positive");
      assert.equal(found.analysisResult.sentimentScore, 0.95);
      assert.equal(found.analysisResult.qualityScore, 0.88);
      assert.deepStrictEqual(found.analysisResult.flags, []);
    });
  });

  test("review analysisResult captures spam flags correctly", async () => {
    const analysisResult = {
      sentiment: "neutral",
      sentimentScore: 0.5,
      qualityScore: 0.1,
      flags: ["spam", "low_quality"],
      summary: "Noi dung ngan, khong co gia tri tham khao.",
      analyzedAt: new Date().toISOString(),
    };
    const review = db.reviews.find((r) => r.id === "review-2");
    review.analysisResult = analysisResult;

    await withServer(async (port) => {
      const res = await fetch(
        `http://127.0.0.1:${port}/api/reviews?productId=prod-galaxy-s25`,
      );
      const body = await res.json();

      assert.equal(res.status, 200);
      const found = body.data.find((r) => r.id === "review-2");
      assert.ok(found);
  ...
  test("second subscriber on same destination also receives notification", async () => {
    await withServer(async (port) => {
      ...
      try {
        ...
        const ws1 = await connectAndSubscribe(
          port,
          "demo-token",
          "/user/queue/order-status",
        );
        const ws2 = await connectAndSubscribe(
          port,
          "demo-token",
          "/user/queue/order-status",
        );
